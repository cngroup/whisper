...
public class Pnt {
	 ...
	 */
	// TODO: change here to enable the weight
	public Pnt bisector1(Pnt point) {
		dimCheck(point);
//		double dist = distanceSq(point);
//		double dr1 = Math.sqrt(wei / Math.PI);
//		double dr2 = Math.sqrt(point.wei / Math.PI);
//		double d = dr1 + dr2;
//		if(dist < point.r + r) {
//			point.r = 0.8 * dist * dr2 / d;
//			r = 0.8 * dist * dr1 / d;
//		}
		
		Pnt pt1 = null, pt2 = null;
		double angle = Math.atan2(point.coord(1) - this.coord(1), point.coord(0) - this.coord(0));	
		if(r == 0D) {
			pt1 = this;
		} else {
			pt1 = new Pnt(r * Math.cos(angle), r * Math.sin(angle));
			pt1 = pt1.add(this);
		}
		if(point.r == 0D) {
			pt2 = point;
		} else { 
			pt2 = new Pnt(point.r * Math.cos(Math.PI + angle), point.r * Math.sin(Math.PI + angle));
			pt2 = pt2.add(point);
		}
		
		Pnt diff = pt1.subtract(pt2);
		Pnt sum = pt1.add(pt2);
		double dot = diff.dot(sum);
		Pnt result = diff.extend(-dot / 2);
		return result;
...
//		return diff.extend(-dot / 2);
	...
	public Pnt bisector(Pnt pt) {
		dimCheck(pt);
		double xi = coord(0);
		double yi = coord(1);
		double xj = pt.coord(0);
		double yj = pt.coord(1);
		double a = 2 * (xj - xi);
		double b = 2 * (yj - yi);
		double c = xi * xi - xj * xj + yi * yi - yj * yj + (pt.r - r);
		
//		System.out.println("Site radii : " + pt.r + ", " + r);
		
		Pnt result = new Pnt(a, b, c);
		return result;
	}
	
	public double distanceSq(Pnt pt) {
		int len = dimCheck(pt);
		double dist = 0;
		for(int i = 0; i < len; ++i) {
			dist += (coord(i) - pt.coord(i)) * (coord(i) - pt.coord(i)); 
		}
		return dist;
	}
