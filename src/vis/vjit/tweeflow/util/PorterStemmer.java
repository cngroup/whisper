 ...
 * The words 'aed', 'eed', 'oed' leave k at 'a' for step 3, and b[k-1] is then
 * out outside the bounds of b.
 *
 * Release 2
 * Similarly,
 * Bug 2 (reported by Steve Dyrdahl 22/2/00) fixed as marked below.
 * 'ion' by itself leaves j = -1 in the test for 'ion' in step 5, and b[j] is
 * then outside the bounds of b.
 *
 * Release 3
...
public class PorterStemmer {
	...
	private int i;
	/* pointer to the end of the stemmed word */
	private int i_end;
	private int j, k;
	private static final int INC = 50;

	// singleton
	private static final PorterStemmer INSTANCE = new PorterStemmer();
	 ...
	 */
	// this is not called by anyone, so remove for now
	// because it expose internal data structure which
	// might be destructively modified. SCK
	//
	...
	public String stem(String _original) {
		if (_original == null) {
			return "";
		}
		_original = _original.trim().toLowerCase();
		String[] words = _original.split("\\W+");
						...
						if (Character.isLetter((char) ch)) {
							...
							while (true) {
								...
								if (!Character.isLetter((char) ch)) {
									...
									String u = s.toString();
									/*
									 * OR, u = new String(s.getResultBuffer(),
									 * 0, s.getResultLength());
									 */
									System.out.print(u);
									break;
